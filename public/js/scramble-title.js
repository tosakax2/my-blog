 ...
 */
export function scrambleTitle({ selector = "h1[data-scramble]", letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()-_=+[]{};:'\",.<>?/|\\~", interval = 40, totalDuration = 800 } = {}) {
  const els = document.querySelectorAll(selector);

  els.forEach((el) => {
    const htmlEl = el;
    const finalText = htmlEl.dataset.scrambleText || htmlEl.textContent || "";
    const length = finalText.length;
    let display = Array.from(finalText);
    let fixed = Array(length).fill(false);
    let scrambleTimer;

    const stepsToFix = [...Array(length).keys()].filter((i) => finalText[i] !== " ");
